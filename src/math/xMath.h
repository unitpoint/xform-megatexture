...
#define SEC2MS(t)				(xMath::FtoiFast((t) * xMath::M_SEC2MS))
#define MS2SEC(t)				((t) * xMath::M_MS2SEC)

#define	ANGLE2SHORT(x)			(xMath::FtoiFast((x) * 65536.0f / 360.0f) & 65535)
#define	SHORT2ANGLE(x)			((x) * (360.0f / 65536.0f))

#define	ANGLE2BYTE(x)			(xMath::FtoiFast((x) * 256.0f / 360.0f) & 255)
...
template<class T> X_INLINE T	Max(const T& x, const T& y) { return (x > y) ? x : y; }
template<class T> X_INLINE T	Min(const T& x, const T& y) { return (x < y) ? x : y; }
template<class T> X_INLINE int	MaxIndex(const T& x, const T& y) { return  (x > y) ? 0 : 1; }
template<class T> X_INLINE int	MinIndex(const T& x, const T& y) { return (x < y) ? 0 : 1; }

template<class T> X_INLINE T	Max3(const T& x, const T& y, const T& z) { return (x > y) ? ((x > z) ? x : z) : ((y > z) ? y : z); }
template<class T> X_INLINE T	Min3(const T& x, const T& y, const T& z) { return (x < y) ? ((x < z) ? x : z) : ((y < z) ? y : z); }
template<class T> X_INLINE int	Max3Index(const T& x, const T& y, const T& z) { return (x > y) ? ((x > z) ? 0 : 2) : ((y > z) ? 1 : 2); }
template<class T> X_INLINE int	Min3Index(const T& x, const T& y, const T& z) { return (x < y) ? ((x < z) ? 0 : 2) : ((y < z) ? 1 : 2); }

template<class T> X_INLINE T	Sign(const T& f) { return (f > 0) ? 1 : ((f < 0) ? -1 : 0); }
template<class T> X_INLINE T	Square(const T& x) { return x * x; }
template<class T> X_INLINE T	Cube(const T& x) { return x * x * x; }
template<class T> X_INLINE T	Clamp(const T& x, const T& min, const T& max) 
{ 
  if(x < min) return min;
  if(x > max) return max;
  return x; 
...
public:
  ...
  static float				EpsRound(float f, float eps);
	...
	static int					FloatHash(float *array, const int numFloats);
	...
	static const float WORLD_INFINITY;			// huge world number which should be larger than any valid number in world
...
X_INLINE float xMath::EpsRound(float f, float eps)
{ 
  return floorf(f / eps + 0.5f) * eps; 
}

X_INLINE float xMath::RSqrt(float x) {
...
X_INLINE float xMath::InvSqrt16(float x) {
...
X_INLINE float xMath::InvSqrt(float x) {
...
X_INLINE double xMath::InvSqrt64(float x) {
	dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
X_INLINE float xMath::Sqrt16(float x) {
	return x * InvSqrt16(x);
}

X_INLINE float xMath::Sqrt(float x) {
	return x * InvSqrt(x);
}

X_INLINE double xMath::Sqrt64(float x) {
	return x * InvSqrt64(x);
}

X_INLINE float xMath::Sin(float a) {
	return sinf(a);
}

X_INLINE float xMath::Sin16(float a) {
	float s;
...
X_INLINE double xMath::Sin64(float a) {
	return sin(a);
}

X_INLINE float xMath::Cos(float a) {
	return cosf(a);
}

X_INLINE float xMath::Cos16(float a) {
	float s, d;
...
X_INLINE double xMath::Cos64(float a) {
	return cos(a);
}

X_INLINE void xMath::SinCos(float a, float &s, float &c) {
#ifdef _WIN32
	_asm {
		fld		a
		fsincos
...
X_INLINE void xMath::SinCos16(float a, float &s, float &c) {
	float t, d;
...
X_INLINE void xMath::SinCos64(float a, double &s, double &c) {
#ifdef _WIN32
	_asm {
		fld		a
		fsincos
...
X_INLINE float xMath::Tan(float a) {
	return tanf(a);
}

X_INLINE float xMath::Tan16(float a) {
	float s;
	bool reciprocal;
...
X_INLINE double xMath::Tan64(float a) {
	return tan(a);
}

X_INLINE float xMath::ASin(float a) {
	if (a <= -1.0f) {
		return -HALF_PI;
	}
	if (a >= 1.0f) {
...
X_INLINE float xMath::ASin16(float a) {
...
X_INLINE double xMath::ASin64(float a) {
	if (a <= -1.0f) {
		return -HALF_PI;
	}
	if (a >= 1.0f) {
...
X_INLINE float xMath::ACos(float a) {
	if (a <= -1.0f) {
		return PI;
	}
	if (a >= 1.0f) {
...
X_INLINE float xMath::ACos16(float a) {
	if (FLOATSIGNBITSET(a)) {
		if (a <= -1.0f) {
			return PI;
		}
...
X_INLINE double xMath::ACos64(float a) {
	if (a <= -1.0f) {
		return PI;
	}
	if (a >= 1.0f) {
...
X_INLINE float xMath::ATan(float a) {
	return atanf(a);
}

X_INLINE float xMath::ATan16(float a) {
	float s;
...
X_INLINE double xMath::ATan64(float a) {
	return atan(a);
}

X_INLINE float xMath::ATan(float y, float x) {
	return atan2f(y, x);
}

X_INLINE float xMath::ATan16(float y, float x) {
	float a, s;
...
X_INLINE double xMath::ATan64(float y, float x) {
	return atan2(y, x);
}

X_INLINE float xMath::Pow(float x, float y) {
	return powf(x, y);
}

X_INLINE float xMath::Pow16(float x, float y) {
	return Exp16(y * Log16(x));
}

X_INLINE double xMath::Pow64(float x, float y) {
	return pow(x, y);
}

X_INLINE float xMath::Exp(float f) {
	return expf(f);
}

X_INLINE float xMath::Exp16(float f) {
...
X_INLINE double xMath::Exp64(float f) {
	return exp(f);
}

X_INLINE float xMath::Log(float f) {
	return logf(f);
}

X_INLINE float xMath::Log16(float f) {
	int i, exponent;
	float y, y2;
...
X_INLINE double xMath::Log64(float f) {
	return log(f);
}

X_INLINE int xMath::IPow(int x, int y) {
	int r; for(r = x; y > 1; y--) { r *= x; } return r;
}

X_INLINE int xMath::ILog2(float f) {
	return (((*(int*)&f) >> IEEE_FLT_MANTISSA_BITS) & ((1 << IEEE_FLT_EXPONENT_BITS) - 1)) - IEEE_FLT_EXPONENT_BIAS;
}

X_INLINE int xMath::ILog2(int i) {
	return ILog2((float)i);
}

X_INLINE int xMath::BitsForFloat(float f) {
	return ILog2(f) + 1;
}

X_INLINE int xMath::BitsForInteger(int i) {
	return ILog2((float)i) + 1;
}

X_INLINE int xMath::MaskForFloatSign(float f) {
	return (*(int*)&f) >> 31;
}

X_INLINE int xMath::MaskForIntegerSign(int i) {
	return (i >> 31);
}

X_INLINE int xMath::FloorPowerOfTwo(int x) {
	return CeilPowerOfTwo(x) >> 1;
}

X_INLINE int xMath::CeilPowerOfTwo(int x) {
	x--;
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
X_INLINE bool xMath::IsPowerOfTwo(int x) {
	return (x & (x - 1)) == 0 && x > 0;
}

X_INLINE int xMath::BitCount(int x) {
	x -= ((x >> 1) & 0x55555555);
...
X_INLINE int xMath::BitReverse(int x) {
...
X_INLINE int xMath::Abs(int x) {
   int y = x >> 31;
   return ((x ^ y) - y);
}

X_INLINE float xMath::Fabs(float f) {
	int tmp = *(int*)&f;
	tmp &= 0x7FFFFFFF;
	return *(float*)&tmp;
}

X_INLINE float xMath::Floor(float f) {
	return floorf(f);
}

X_INLINE float xMath::Ceil(float f) {
	return ceilf(f);
}

X_INLINE float xMath::Rint(float f) {
	return floorf(f + 0.5f);
}

X_INLINE int xMath::Ftoi(float f) {
	return (int) f;
}

X_INLINE int xMath::FtoiFast(float f) {
#ifdef _WIN32
	int i;
	__asm fld		f
...
X_INLINE unsigned long xMath::Ftol(float f) {
	return (unsigned long) f;
}

X_INLINE unsigned long xMath::FtolFast(float f) {
#ifdef _WIN32
...
X_INLINE signed char xMath::ClampChar(int i) {
	if (i < -128) {
		return -128;
	}
	if (i > 127) {
...
X_INLINE signed short xMath::ClampShort(int i) {
	if (i < -32768) {
		return -32768;
	}
	if (i > 32767) {
...
X_INLINE int xMath::ClampInt(int min, int max, int value) {
	if (value < min) {
		return min;
	}
	if (value > max) {
...
X_INLINE float xMath::ClampFloat(float min, float max, float value) {
	if (value < min) {
		return min;
	}
	if (value > max) {
...
X_INLINE float xMath::AngleNormalize360(float angle) {
...
X_INLINE float xMath::AngleNormalize180(float angle) {
...
X_INLINE float xMath::AngleDelta(float angle1, float angle2) {
	return AngleNormalize180(angle1 - angle2);
}

X_INLINE int xMath::FloatHash(float *array, const int numFloats) {
	int i, hash = 0;
	const int *ptr;
